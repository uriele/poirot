...
fn normalize_abs_id(id_url: &str) -> Result<(String, Option<String>), QueryError> {
    let tail = id_url
        ...
        .ok_or(QueryError::UnexpectedError(
            "Invalid arXiv ID URL".to_string(),
        ))?
        .trim();
    ...
    let id = caps
        .name("id")
        .ok_or_else(|| QueryError::UnexpectedError("Missing id group".to_string()))?
        .as_str()
        .to_string();
    ...
    D: serde::Deserializer<'de>,
{
    ...
    let (id, _) = normalize_abs_id(&raw).map_err(serde::de::Error::custom)?;
    Ok(id)
}
#[allow(dead_code)]
...
mod test {
    ...
    fn test_parse_atom_entries() {
        ...
        assert_eq!(
            entry.title.as_ref().unwrap().to_string(),
            "Sample Title".to_string()
        );
        assert_eq!(
            entry.summary.as_ref().unwrap().to_string(),
            "Sample abstract text.".to_string()
        );
        assert_eq!(entry.authors.len(), 2);
        assert_eq!(
            entry.authors[0].name.as_ref().unwrap().to_string(),
            "First Author".to_string()
        );
        assert_eq!(entry.categories.len(), 2);
        assert_eq!(entry.categories[0].term, "cs.AI".to_string());
        assert_eq!(
            entry.comment.as_ref().unwrap().to_string(),
            "10 pages, 5 figures".to_string()
        );
    }
}
