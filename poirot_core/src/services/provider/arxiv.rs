use super::arxivquery_ast::{ast, interpret, parse};
...
use crate::domain::provider::{LiteratureProvider, SearchQuery};
...
fn create_rate_limiter() -> Result<RateLimiter<NotKeyed, InMemoryState, DefaultClock>, QueryError> {
    ...
    Ok(RateLimiter::direct_with_clock(
        quota,
...
impl ArxivQuery {
    pub fn write_query(&self) -> Result<String, QueryError> {
        ...
        if let (Some(from_date), Some(to_date)) = (from_date, to_date) {
            if from_date > to_date {
                return Err(QueryError::WrongFormatting(
                    "from_date cannot be greater than to_date".to_string(),
                ));
            }
        }
        ...
        (!query_body.is_empty())
            .then_some(())
            .ok_or(QueryError::WrongFormatting(
                "Arxiv query body cannot be empty".to_string(),
            ))?;
...
impl LiteratureProvider for ArxivProvider {
    ...
    async fn search(
        &self,
        query_read: Self::SearchQueryType,
    ) -> Result<Self::SearchResultType, Box<dyn Error + Send + Sync>> {
        ...
        let url_base = Arc::new(format!(
            "{}?{}{}&{}{}&{}{}",
            arxiv_text::BASEURL,
            arxiv_text::SEARCH_QUERY,
            arxiv_query.as_str(),
            arxiv_text::SORT_BY,
            sort_by,
            arxiv_text::MAX_RESULTS,
            &query_read.per_page.to_string()
        ));
        ...
        fn pagination(
            ...
            to_page: Option<u32>,
        ) -> Result<impl Iterator<Item = u32>, QueryError> {
            match (from_page, to_page) {
                (None, Some(to)) => Ok(0..to),
                (Some(from), None) => Ok(from..from),
                (Some(from), Some(to)) => {
                    if to < from {
                        return Err(QueryError::WrongFormatting(
                            "to_page cannot be less than from_page".to_string(),
                        ));
                    }
                    Ok(from..to)
                }
        ...
        for pag in pagination(query_read.from_page, query_read.to_page)? {
            ...
            set.spawn(async move {
                ...
                if !response.status().is_success() {
                    return Err(QueryError::UnexpectedError(format!(
    ...
    async fn fetch_by_source_id(
        &self,
        _source_id: &str,
    ) -> Result<Box<dyn LibraryItem>, Box<dyn Error + Send + Sync>> {
        // Implementation for fetching a literature item by source ID from arXiv
        Err(Box::new(QueryError::UnexpectedError(
...
mod test {
    ...
    fn test_build_arxiv_query() {
        ...
        assert_eq!(
            arxiv_query_str,
            r#"all:"quantum computing"+AND+au:"John H. Doe""#
        );
    }
    ...
    async fn concurrent_searches_are_rate_limited() {
        ...
        let _ = provider.search(query).await;
