...
impl Title{
    pub fn parse(title_str: &str) -> Result<Self,QueryError> {
        ...
        match title_str.is_empty(){
            true => return Err(QueryError::WrongFormatting("Title cannot be empty".to_string())),
            false => (),
        }
...
pub trait LibraryItem {
    fn has_pages(&self) -> bool {false}
    fn has_doi(&self) -> bool {false}
    fn has_website(&self) -> bool {false}
}
...
mod test{
    ...
    fn test_article_builder_success(){
        ...
        let authors= vec![Author::builder()
        ...
        assert_eq!(article.doi.unwrap().to_string(),"10.1000/xyz123".to_string());
        assert_eq!(article.authors.len(),1);
        assert_eq!(article.authors.iter().next().unwrap().to_string(),"Alice B. Example".to_string());
        assert_eq!(article.abstract_text.unwrap(),"This is an abstract".to_string());
    }   
    #[test]
